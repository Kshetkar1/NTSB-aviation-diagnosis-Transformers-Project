...
| top_k neighbors for severity readout | eval `--sev-topk` | SELECTED on internal split (queries 2002–2006, pool 1982–2001): k=25 | training window only |
...
It is used only to select/sanity-check retrieval hyperparameters. The test
window (2007–2019) is disjoint from both by accident ID and by year, verified
by `tests/heldout_leak_audit.py` (0 / 296 overlaps).
