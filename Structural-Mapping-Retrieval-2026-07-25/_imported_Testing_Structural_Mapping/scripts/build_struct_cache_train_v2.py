#!/usr/bin/env python3
"""
Build JSONL cache of incident_struct_v2 (causal_chain) for train ev_ids (OpenAI).

Same train ids as v1; output is separate (struct_train_v2.jsonl). Used for A2 eval.

Usage:
  cd <repo_root>
  export OPENAI_API_KEY=...
  python Testing_Structural_Mapping/scripts/build_struct_cache_train_v2.py --limit 50
...
from config import TRAIN_MERGED_DATA_PATH, LLM_MODEL  # noqa: E402

from extract_struct_v2 import extract_struct_from_incident_v2  # noqa: E402
from io_cache import append_jsonl, iter_existing_ev_ids  # noqa: E402
from paths import DEFAULT_STRUCT_CACHE_V2_PATH, TRAIN_IDS_PATH  # noqa: E402
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Cache incident_struct_v2 for train ev_ids.")
    ap.add_argument("--output", type=Path, default=DEFAULT_STRUCT_CACHE_V2_PATH)
    ...
    ap.add_argument("--model", default=None, help="Overrides OPENAI_STRUCT_MODEL / default")
    args = ap.parse_args()
    ...
    for i, eid in enumerate(ids, 1):
        ...
        if not inc:
            ...
            continue
        try:
            st = extract_struct_from_incident_v2(inc, eid, model=args.model)
