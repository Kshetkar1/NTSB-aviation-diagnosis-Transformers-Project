...
_cli = argparse.ArgumentParser(description="K-means + LLM labels; writes cluster_label into merged JSON.")
_cli.add_argument(
    "--train",
    action="store_true",
    help="Use train embeddings/map/merged only; update merged_dataset_train.json (not full merged_dataset.json).",
)
...
def precompute_clusters():
    merged_path = TRAIN_MERGED_DATA_PATH if _args.train else REFINED_DATA_PATH
    emb_path = EMBEDDINGS_TRAIN_PATH if _args.train else EMBEDDINGS_PATH
    map_path = EMBEDDINGS_MAP_TRAIN_PATH if _args.train else EMBEDDINGS_MAP_PATH

    print(
        "🚀 Starting Pre-computation of Incident Clusters (Lightweight Version)"
        + (" — TRAIN" if _args.train else "")
        + "...",
        flush=True,
    )
