                ...
                if diagnosis_results and diagnosis_results.get('weighted_causes'):
                    ...
                    for c_name, c_prob in sorted_clusters[:5]:
                        with st.expander(f"📂 {c_name.title()} (P={c_prob:.1%})", expanded=False):
                            ...
                            for cause_info in causes_in_group[:3]:
                                ...
                                cause_text = clean_ntsb_text(cause_info['cause'])
                            ...
                            if 'clusters' in diagnosis_results:
                                ...
                                if cluster_data:
                                    ...
                                    if inc_ids:
                                        st.markdown("---")
                                        st.caption("Typical Event Sequence:")
                                        ...
                                        if mermaid_code:
                                            # Force LR for Diagnosis visual to match user pref
                                            mermaid_code = mermaid_code.replace("graph TD", "graph LR")
                                            components.html(render_mermaid_html(mermaid_code), height=300, scrolling=True)
                    ...
                    final_causes = diagnosis_results['weighted_causes'][:5]
                    for i, cause in enumerate(final_causes, 1):
                        cause_text = clean_ntsb_text(cause['cause'])
                        ...
                        with st.container():
                            ...
                            if cause.get('cluster_breakdown'):
                                breakdown_text = " + ".join([
                                    f"({c['p_cause_cluster']:.2f} × {c['p_cluster_query']:.2f})" 
                                    for c in cause['cluster_breakdown'][:3]
                                ])
                                st.caption(f"Calculation: {breakdown_text} ...")
                        st.divider()
